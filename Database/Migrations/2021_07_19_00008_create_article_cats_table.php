...
class CreateArticleCatsTable extends Migration
{
    ...
    private function create()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('cat_id')->nullable(false)->comment("文章分类ID")->index("cat_id_index");
            $table->timestamps();

            // 设置外键
            $table->foreign('article_id', $this->tableName . "_ibfk_1")->references('id')->on('article');
            $table->foreign('cat_id', $this->tableName . "_ibfk_2")->references('id')->on('article_cat');
        });
        $prefix = DB::getConfig('prefix');
        $qu = "ALTER TABLE " . $prefix . $this->tableName . " comment '文章分类关联表'";
        DB::statement($qu);
    }
}
